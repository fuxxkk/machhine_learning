...
file_layer_1_path = "D:/ML/ml_layer_1_values.txt"
file_layer_2_path = "D:/ML/ml_layer_2_deltas.txt"


def load_db(file_path):
    try:
        load_layer_values = open(file_path, "rb+")  # 读取值
        layer_values = pickle.load(load_layer_values)
        load_layer_values.close()
    except:
        layer_values = list()
    return layer_values


def save_db(file_path, list):
    save_layer_values = open(file_path, "wb+")  # 保存值
    pickle.dump(list, save_layer_values)
    save_layer_values.close()


print("读取结果。。")
layer_1_values = load_db(file_layer_1_path)
layer_2_deltas = load_db(file_layer_2_path)
print("读取完毕。。")
...
print("正在保存学习结果。。。")
save_db(file_layer_1_path, layer_1_values)
save_db(file_layer_2_path, layer_2_deltas)
print("保存完毕。。")
